output "instance_id" {
  ...
  value       = aws_instance.dora.id
}

output "public_ip" {
  description = "Elastic IP — point your DNS A record here."
  value       = aws_eip.dora.public_ip
}
...
output "sslip_domain" {
  ...
  value       = "${replace(aws_eip.dora.public_ip, ".", "-")}.sslip.io"
}

output "ssm_connect_command" {
  description = "Command to open a shell on the instance via SSM Session Manager (no SSH key/port needed)."
  value       = "aws ssm start-session --target ${aws_instance.dora.id} --region ${var.aws_region}"
}

output "security_group_id" {
  description = "Security group attached to the instance."
  value       = aws_security_group.dora.id
}
